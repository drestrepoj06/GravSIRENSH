...
from SRC.Location_encoder.Networks import SIRENNet, LINEARNet
import numpy as np
...
class Scaler:
    ...
    def fit(self, df):
        ...
        if self.mode in ["U", "g_direct", "g_indirect", "U_g_direct", "g_hybrid"]: # "U_g_hybrid", "U_g_indirect"]:
...
class Gravity(pl.LightningModule):
    def __init__(self, model_cfg, scaler, lr=1e-4):
        ...
        if arch == "sirensh":
            ...
            raise ValueError(
                f"Unknown architecture '{arch}'. Expected 'sirensh' or 'linearsh'."
            )
